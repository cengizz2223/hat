  ...
  return (
    <section id="projects" style={{ padding: '100px 80px', background: '#000' }}>
      <div style={{ display: 'flex', gap: `${GAP}px`, alignItems: 'flex-start', paddingTop: `${RISE}px` }}>
        ...
        {/* CENTER — wider + elevated */}
        <div style={{ flex: '1.2', display: 'flex', flexDirection: 'column', gap: `${GAP}px`, marginTop: `-${RISE}px`, position: 'relative', zIndex: 3 }}>
          <Card hov={hov} idx={3} setHov={setHov} style={{ height: `${BIG_H}px`, boxShadow: '0 24px 60px rgba(0,0,0,0.6)' }} />
          <Card hov={hov} idx={4} setHov={setHov} showLabel style={{ height: `${BIG_H}px`, boxShadow: '0 24px 60px rgba(0,0,0,0.6)' }} />
          <Card hov={hov} idx={5} setHov={setHov} showLabel style={{ height: `${BIG_H}px`, boxShadow: '0 24px 60px rgba(0,0,0,0.6)' }} />
        </div>
