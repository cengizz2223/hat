...
export default function CtaFooter() {
  ...
  return (
    <>
      <section id="contact" style={{ position:'relative', overflow:'hidden', textAlign:'center', padding:'140px 24px', background:'#000', borderTop:'1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ position:'absolute', inset:0, zIndex:1, pointerEvents:'none', background:'radial-gradient(ellipse at 50% 50%, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0.7) 100%)' }} />
        <div style={{ position:'absolute', top:0, left:0, right:0, height:'120px', zIndex:1, pointerEvents:'none', background:'linear-gradient(to bottom, #000, transparent)' }} />

        <div style={{ position:'relative', zIndex:2, maxWidth:'680px', margin:'0 auto', display:'flex', flexDirection:'column', alignItems:'center' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'7px 16px', borderRadius:'100px', background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.12)', color:'rgba(255,255,255,0.7)', fontSize:'13px', marginBottom:'32px' }}>
            <span style={{ width:'6px', height:'6px', background:'#fff', borderRadius:'50%' }} />
            Hat Eserleri Satışta
          </div>
          <h2 style={{ fontSize:'clamp(28px,4vw,48px)', fontWeight:600, color:'#fff', letterSpacing:'-1.5px', lineHeight:1.15, marginBottom:'32px' }}>
            Hat sanatıyla ilgileniyor musunuz? Güzelliği evinize taşıyın!
          </h2>
          <Link href="mailto:hello@framebase.design" style={{ padding:'13px 28px', borderRadius:'100px', background:'transparent', border:'1px solid rgba(255,255,255,0.3)', color:'#fff', fontSize:'14px', textDecoration:'none', marginBottom:'28px' }}>
